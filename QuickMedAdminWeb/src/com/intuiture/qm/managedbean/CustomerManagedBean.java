...
import com.intuiture.qm.bean.CustomerBean;
...
public class CustomerManagedBean extends CustomerBean {
	...
	public SearchJson getsearchComboAction(String typeOfField) {
		...
		try {
			String[] searchFieldLabels = { "First Name", "Last Name", "Address", "Email", "Phone Number", "Created On" };
			String[] searchFieldValues = { "firstName", "lastName", "address", "emailId", "phoneNumber", "createdOn" };
			SerConstants[] searchFieldType = { SerConstants.STRING, SerConstants.STRING, SerConstants.STRING, SerConstants.STRING, SerConstants.NUMBER, SerConstants.DATE };
	...
	public String navigationAction() {
		...
		if (getNavigationTagId().startsWith("f")) {
			...
			offSetValue = getGridInfoJson().getNoOfRecordSize().equals(0) ? 0
					: (getGridInfoJson().getNoOfRecordSize() % getGridInfoJson().getRecordsPerPage()) == 0 ? (getGridInfoJson().getNoOfRecordSize() - getGridInfoJson()
							.getRecordsPerPage()) : (getGridInfoJson().getNoOfRecordSize() - (getGridInfoJson().getNoOfRecordSize() % getGridInfoJson()
							.getRecordsPerPage()));
		}
	...
	public void getCurrentPage() {
		...
		String currentRecordPage = listSize == 0 ? "" : "Page " + ((offSetValue / count) + 1) + " Of "
				+ (listSize.equals(0) ? 1 : (listSize % count == 0) ? (listSize / count) : (listSize / count) + 1);
